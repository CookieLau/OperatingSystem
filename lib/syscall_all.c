...
int sys_set_pgfault_handler(int sysno, u_int envid, u_int func, u_int xstacktop)
{
	...
	if ((ret = envid2env(envid,&env,0)) < 0) {
		printf("can not find any env mapped envid!\n");
		return ret;
	}
...
int sys_mem_alloc(int sysno, u_int envid, u_int va, u_int perm)
{
	...
	if (va >= UTOP || va < 0) {
		printf("error:va >= UTOP in sys_mem_alloc\n");
		return -E_INVAL;
	}
	if ((perm & PTE_V) == 0x0 || (perm & PTE_COW) != 0x0) {
		printf("error:PTE_V or PTE_COW is not right in sys_mem_alloc\n");
		return -E_INVAL;
	}
	if ((ret = page_alloc(&ppage)) != 0) {
		printf("error:page_alloc failed in sys_mem_alloc\n");
		return ret;
	}
	if ((ret = envid2env(envid,&env,0)) != 0) {
		printf("error: cant find any env named envid in sys_mem_alloc\n");
		return ret;
	}
	if ((ret = page_insert(env->env_pgdir,ppage,va,perm)) != 0) {
		printf("error: cant insert page in sys_mem_alloc\n");
		return ret;
	}
	return 0;
}
				...
				u_int perm)
{
	...
	if (srcva >= UTOP || srcva < 0 || dstva >= UTOP || dstva < 0) {
		printf("error:va is not allowed in sys_mem_map\n");
		return -E_INVAL;
	}
	//if ((perm & PTE_V) == 0x0 || (perm & PTE_COW) != 0x0) {	
	if ((perm & PTE_V) == 0x0) {
		printf("error:PTE_V or PTE_COW is not right in sys_mem_map\n");
		return -E_INVAL;
	}
	if ((ret = envid2env(srcid,&srcenv,0)) != 0) {
		printf("error: cant find any srcenv named envid in sys_mem_map\n");
		return ret;
	}
	if ((ret = envid2env(dstid,&dstenv,0)) != 0) {
		printf("error: cant find any dstenv named envid in sys_mem_map\n");
		return ret;
	}	
	if ((ppage = page_lookup(srcenv->env_pgdir,round_srcva,&ppte)) == 0) {
		printf("error: cant find any page mapped at srcva in sys_mem_map\n");
	...
	if ((ret = page_insert(dstenv->env_pgdir,ppage,round_dstva,perm)) != 0) {
		printf("error: cant insert page in dstva in sys_mem_map\n");
		return ret;
	}
	return 0;
}
...
int sys_mem_unmap(int sysno, u_int envid, u_int va)
{
	...
	if (va >= UTOP || va < 0) {
		printf("error: va is not allowed in sysys_mem_unmap\n");
		return -E_INVAL;
	}

	if ((ret = envid2env(envid,&env,0)) != 0) {
		printf("error: cant find any env mapped envid in sys_mem_unmap\n");
		return ret;
	}
...
int sys_env_alloc(void)
{
	...
	//printf("success alloc env in syscall_all.c line 293\n");
	return e->env_id;
...
int sys_set_env_status(int sysno, u_int envid, u_int status)
{
	...
	int ret;
	if ((ret = envid2env(envid,&env,0)) < 0) {
		printf("can not find any env mapped envid in sys_set_env_status\n");
		return ret;
	}
	u_int oldstatus = env->env_status;
	if (!(oldstatus == ENV_RUNNABLE || oldstatus == ENV_NOT_RUNNABLE || oldstatus == ENV_FREE)) {
		return -E_INVAL;
	}
	env->env_status = status;
	if (oldstatus != ENV_RUNNABLE && status == ENV_RUNNABLE) {
		LIST_INSERT_HEAD(&env_sched_list[sched_list_id^1],env,env_sched_link);
	}
	return 0;
...
void sys_ipc_recv(int sysno, u_int dstva)
{
	if (dstva >= UTOP || dstva < 0) {
		printf("error: dstva is invalid in sys_ipc_recv\n");
		return -E_INVAL;
	}
					 ...
					 u_int perm)
{
	...
	if ((r = envid2env(envid, &e, 0)) != 0) {
		printf("error: can not find any env mapped envid in sys_ipc_can_send\n");
		return r;
	}
...
{
        // Your code here
	if ((dev >= 0x10000000 && dev < 0x10000020 && dev+len <= 0x10000020) ||
	    (dev >= 0x13000000 && dev < 0x13004200 && dev+len <= 0x13004200) ||
	    (dev >= 0x15000000 && dev < 0x15000200 && dev+len <= 0x15000200)) {	
		bcopy((void *)va,(void *)(0xA0000000+dev),len);
		return 0;//success
	}
	printf("wrong_length in sys_write_dev!\n");
	return -E_INVAL;
}
...
int sys_read_dev(int sysno, u_int va, u_int dev, u_int len)
{
	...
	printf("wrong_length in sys_read_dev!\n");
	return -E_INVAL;
}
