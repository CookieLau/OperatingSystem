...
env_setup_vm(struct Env *e) {
 ...
 /*Step 3: Copy kernel's boot_pgdir to pgdir. */
	...
	//bcopy(boot_pgdir,pgdir+PDX(UTOP),(0xffffffff - PDX(UTOP))); 
...
{	
	...
	//first copy with the offset
	if (offset > 0) {
		if ((r = page_alloc(&p)) == -E_NO_MEM) {
			return r;
		}
		if ((r = page_insert(env->env_pgdir,p,va,perm)) != 0) {
			printf("page_insert failed in load_icode_mapper.\n");
			return r;
		}
	...
	//copy with the middle full blocks
	for (; i+BY2PG < bin_size; i += BY2PG) { 
	/* Hint: You should alloc a page and increase the reference count of it. */
		if ((r = page_alloc(&p)) == -E_NO_MEM) {
			return -E_NO_MEM;
		}
		if ((r = page_insert(env->env_pgdir,p,tempva,perm)) < 0) {
			return -E_NO_MEM;
		}		
...
env_create_priority(u_char *binary, int size, int priority)
{
    ...
    //printf("success creating env here in env.c in 391 line\n");
    return ;
