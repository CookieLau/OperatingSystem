 ...
 *  Do NOT map anything into the user portion of the environment's virtual*/ 
 /***Your Question Here***/
static int env_setup_vm(struct Env *e) {
  ...
  *pgdir is the page directory of Env e, assign value for it. */
	...
	if ((r = page_alloc(&p)) == -E_NO_MEM) { //allocate a page dictionary
  		panic("env_setup_vm - page alloc error\n");
                return r;
      	}
	p->pp_ref ++;//add its reference 
	pgdir = (Pde *)page2kva(p);//assign value for pgdir
...
/* Overview:
 *  Allocates and Initializes a new environme
...
env_alloc(struct Env **new, u_int parent_id) {
	...
	if ((e = LIST_FIRST(&env_free_list)) == NULL) { 
	    ...
	    *The function mainly maps the kernel address to this new Env address. */
		...
		printf("error in env_alloc: -E_NO_MEM\n");
		return r;
	}
	...
	//once an env is runnable, insert it into the tail of the first env_sched_list
	//LIST_INSERT_TAIL(&env_sched_list[1-sched_list_id],e,env_sched_link);
...
/* Overview:
 *   This is a call back function for kernel's elf loade
 * Elf loader extracts each segment of the given binary image.
 * Then the loader calls this function to map each segmen
 * at correct virtual address
 
 *   `bin_size` is the size of `bin`. `sgsize` is the
 * segment size in memor
 *
 * Pre-Condition
 *   bin can't be NULL
...
{	
	...
	/*
	1: load all content of bin into memory. */
	for (i = 0; i < bin_size; i += BY2PG) { 
	/* Hint: You should alloc a page and increase the reference count of it. */
		if ((r = page_alloc(&p)) == -E_NO_MEM) {
			return -E_NO_MEM;
		}
		//p->pp_ref ++; //page_insert will add this reference
		if (i == 0) {
			bcopy(bin,(char *)page2kva(p)+offset,(bin_size < BY2PG - offset)? (bin_size):BY2PG - offset);
		} else {
			bcopy(bin+i-offset,(char *)page2kva(p),(bin_size-i+offset < BY2PG)? (bin_size - i+offset):BY2PG);
		}
 ...
 *  This function loads the complete binary image by using elf loade
 *  into the environment's user memory. The entry point of the binary image
 *  is given by the elf loader. And this function maps one page for th
 *  program's initial stack at virtual address USTACKTOP - BY2PG
 
 * Hints: 
 *  All mappings are read/write including those of the text segme
 *  You may us
...
/* Hint:
	 *  You must figure out which permissions you'll nee
	 *  for the different mappings you create.
	 *  Remember½C that the binary image is an a.out format image,
    ...
    /*Step 1: alloc a page. */
	if ((r=page_alloc(&p)) < 0) {
		printf("fail to alloc_page in load_icode function!");
		return;
	}
  ...
  /*Step 2: Use appropriate perm to set initial stack for new Env. */
	...
	perm = PTE_R;
  	if ((r = page_insert(e->env_pgdir,p,USTACKTOP-BY2PG,perm)) < 0) {
		printf("error in function:load_icode in page_insert");
	}
  /*Step 3:load the binary by using elf loader. */
	if ((r=load_elf(binary,size,&entry_point,e,load_icode_mapper)) < 0) {
		printf("Error in function:load_icode in load_elf");
		return ;
	}
    /***Your Question Here***/
    	e->env_status = ENV_RUNNABLE;
	LIST_INSERT_HEAD(&env_sched_list[1-sched_list_id],e,env_sched_link);
...
/* Overview:
 *  Allocates a new env with env_alloc, loads the named elf binary int
...
env_create_priority(u_char *binary, int size, int priority)
{
    ...
    if (env_alloc(&e,0) < 0) {
	printf("env_alloc failed! in function: env_create_priority");
	return ;
    }
