...
// reverse: transform little endian to little endian.This function will change p directly.
...
void init_disk() {
    ...
    for(i = 0; i < nbitblock; ++i) {
        memset(disk[2+i].data, 0xff, NBLOCK/8);//since block.data store in type of byte
    }
    if(NBLOCK != nbitblock * BY2BLK) {//actually is nbitblock * by2blk > nblock,means bmap has vacancy
        diff = NBLOCK % BY2BLK / 8;//calculate the last used block numbers
        memset(disk[2+(nbitblock-1)].data+diff, 0x00, BY2BLK - diff);//mark all as 0
    }
    ...
    strcpy(super.s_root.f_name, "/");//root dir, using /
}
...
void save_block_link(struct File *f, int nblk, int bno)
{
    ...
    if(nblk < NDIRECT) {
	    ...
	    //since f_indirect == 0, 
	    //	which means we have no indirect yet
	    //		so we create an index(pointer) 
	    //			which points to a new block with first ten pointers unused 
	    //				for indrect pointing
            f->f_indirect = next_block(BLOCK_INDEX);//only index
        }
...
int make_link_block(struct File *dirf, int nblk) {
    save_block_link(dirf, nblk, nextbno);//nextbno is next available block
    dirf->f_size += BY2BLK;
...
struct File *create_file(struct File *dirf) {
    ...
    if (nblk <= NDIRECT) {//when less than 10,we use direct 
...
void write_file(struct File *dirf, const char *path) {
    ...
    const char *fname = strrchr(path, '/');//strrchr:find the first match char from the right side
    if(fname)//since we now point to '/',so we need to add 1 to delete '/'
        fname++;
    else
        fname = path;//if fname == NULL means no '/' in path,
...
void write_directory(struct File *dirf, char *name) {
    // Your code here
    struct File *ffile = create_file(dirf);
    ...
    strcpy(ffile->f_name,filename);
    ffile->f_size = 0;
    ffile->f_type = FTYPE_DIR;
    return ;
}
