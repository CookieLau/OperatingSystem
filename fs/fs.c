...
diskaddr(u_int blockno)
{
	if (super && blockno >= super->s_nblocks) {
		user_panic("blockno exceeds in fs/fs.c/disaddr");
	}
	return DISKMAP + blockno*BY2PG;
}
...
map_block(u_int blockno)
{
	...
	int ret;
	if ((ret = block_is_mapped(blockno)) != 0) {
		return 0;//success
	}
	// Step 2: Alloc a page of memory for this block via syscall.
	ret = syscall_mem_alloc(0,diskaddr(blockno),PTE_V|PTE_R);//0 means current env
	return ret;
}
...
unmap_block(u_int blockno)
{
	...
	//	user_panic("not found block in unmap_block\n");
		return;//success
	}
...
dir_lookup(struct File *dir, char *name, struct File **file)
{
	...
	if (debug)
		writef("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF\n");
	return -E_NOT_FOUND;
}
...
//Return:
//	Return 0 on success
//	Return -E_BAD_PATH on file_name_length exceeds
//	Return -E_NOT_FOUND on not finding corresponding file
	int
walk_path(char *path, struct File **pdir, struct File **pfile, char *lastelem)
{
	...
	*pfile = 0;
	...
	while (*path != '\0') {
		...
		user_bcopy(p, name, path - p);//bcopy(src,dst,len);
		name[path - p] = '\0';
		path = skip_slash(path);
		...
		if ((r = dir_lookup(dir, name, &file)) < 0) {//dir_lookup is to find a *file* in the dir
...
file_remove(char *path)
{
	...
	writef("WARNING: REMOVING A FILE!\n---------------------------------------------------------------\n");
