...
static void *alloc(u_int n, u_int align, int clear)
{
    ...
    if (clear) {
    	//printf("the alloced_mem is %x,the size is %d\n");
...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
    ...
    u_long pdx = PDX(va);
    //u_long ptx = PTX(va);
    pgdir_entryp = (Pde *)pgdir + pdx;
    u_long pgdir_entryp_va = KADDR(*pgdir_entryp);
    pgtable = (Pte *) PTE_ADDR((Pte *)pgdir_entryp_va);
	...
	if (create == 1) {
	    pgtable = (Pte *)alloc(BY2PG,BY2PG,1);//alloc the size of a page
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    ...
    if (size % BY2PG != 0) {
	panic("size is not multiple of  BY2PG!\n");
	return ;
    }
	...
	*pgtable_entry = (pa+i) | perm | PTE_V;//since the pgtable_entry store the physical address of page
    }
    
    u_long pdx = PDX(va);
    pgdir[pdx] = pgdir[pdx] | perm | PTE_V;//since the pg_table is valid, pg_dir should validate itself too
    return ;
}
...
page_init(void)
{
    ...
    LIST_INIT(&page_free_list);//the argument is pointer to a structure


    /* Step 2: Align `freemem` up to multiple of BY2PG. */
    freemem = ROUND(freemem,BY2PG);//defined in include/mmu.h

    /* Step 3: Mark all memory blow `freemem` as used(set `pp_ref`
     * filed to 1) */
    int i;
    int max = PADDR(freemem) / BY2PG;
    if (max > npage)
    	panic("In page_init:page excels npage!");
    for (i = 0;i < max;i++) {
	pages[i].pp_ref = 1;
    }
    //printf("reached 197\n");
...
//	printf("hhhhhhhi=%d %d\n",i,npage);
    ...
    //printf("here done the page_init\n");
    //check if the page is decreasing hhhhhhhhhhhhhhhh
...
page_alloc(struct Page **pp)
{
	...
	*pp = ppage_temp;
     ...
     * Hint: use `bzero`. */
    //the argumnent of bzero is the virtual address, so we need to transfer
        u_long	pp_va = page2kva(ppage_temp);
    	bzero((void *)pp_va,BY2PG);
    	return 0;
    } else {
	return -E_NO_MEM;
    }
...
page_free(struct Page *pp)
{
    ...
    if (pp->pp_ref >0) {
	return ;//do nothing since something refers to it
    } else if (pp->pp_ref == 0) {
    /* Step 2: If the `pp_ref` reaches to 0, mark this page as free and return. */
	//HEAD or TAIL? it is a question
...
get_page_status(int pa) {
	...
	u_long pp_va = page2kva(temp);
	if (pp_va >= freemem) {
		status = 1;
	} else {
		struct Page *link = LIST_FIRST(&page_free_list);
		while (link) {
			if (pp_va == page2kva(link)) {
				status = 2;
				break;
			}
		}
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
    ...
    u_long pdx = PDX(va);
    u_long ptx = PTX(va);
    //u_long pte_address = PTE_ADDR(va);
    pgdir_entryp = (Pde *)pgdir + pdx;
    u_long pgdir_entryp_va = KADDR(*pgdir_entryp);
    pgtable = (Pte *)PTE_ADDR(pgdir_entryp_va);
    ...
    if ((*pgdir_entryp & PTE_V) == 0x0) {
    	if (create == 1) {
    		int return_value = page_alloc(&ppage);
		if (return_value != 0) {
			*ppte = 0x0;
			return -E_NO_MEM;
		} else {
			...
			pgtable = (Pte *)ppage_va;
			*pgdir_entryp = ppage_pa | PTE_V | PTE_R;
			ppage->pp_ref ++;
		}
	}else {
		*ppte = 0x0;
...
page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
    ...
    *pgtable_entry = page2pa(pp) | PERM;//the pgtable_entry stores the physical address of page at higher 20 bits
    pp->pp_ref ++;
...
physical_memory_manage_check(void)
{
    ...
    //printf("reached physical check 481\n");
...
page_check(void)
{
    ...
    assert(page_alloc(&pp1) == 0);
    assert(page_alloc(&pp2) == 0);
   // printf("here arrived line:615\n");
    assert(pp0);
    assert(pp1 && pp1 != pp0);
    assert(pp2 && pp2 != pp1 && pp2 != pp0);
   // printf("here arrived line:619\n");
    // temporarily steal the rest of the free pages
    fl = page_free_list;
    // now this page_free list must be empty!!!!
    LIST_INIT(&page_free_list);
    ...
    // should be no free memory
    assert(page_alloc(&pp) == -E_NO_MEM);
   // printf("here arrived line:627\n");
 
    // there is no free memory, so we can't allocate a page table
    assert(page_insert(boot_pgdir, pp1, 0x0, 0) < 0);
   // printf("here succeed page_insert\n");
