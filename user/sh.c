	...
	for(;;){
		...
		case '<':
			...
			if ((r = open(t, O_RDONLY)) < 0) {
				user_panic("can not open for reading in runcmd\n");
				return;
			}
			fdnum = r;
			dup(fdnum, 0);
			if ((r = close(fdnum)) < 0) {
				user_panic("can not close the reading fd in < csae\n");
				return;
			}
			goto runit;
		...
		case '>':	
			...
			if (Debug)
				writef("gettoken t:%s in > case\n",t);
			// Your code here -- open t for writing,
			// dup it onto fd 1, and then close the fd you got.
			if ((r = open(t, O_RDWR)) < 0) {
				user_panic("can not open for writing in runcmd\n");
				return;
			}
			fdnum = r;
			dup(fdnum, 1);
			if ((r = close(fdnum)) < 0) {
				user_panic("can not close the writing fd in > csae\n");
				return;
			}
			goto runit;
			//user_panic("> redirection not implemented");
		...
		case '|':
			...
			if ((r = pipe(p)) < 0) {
				user_panic("error fail to alloc a fd in sh.c\n");
				exit();
			}
			//	Then fork.
			int pi = fork();
