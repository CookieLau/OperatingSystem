...
pipe(int pfd[2])
{
	...
	if (debug)
		writef("here alloc a fd for pipe!--------------------------\n");
...
_pipeisclosed(struct Fd *fd, struct Pipe *p)
{
	...
	//struct Page *pfd = (struct Page*) va2page(curenv->env_pgdir, fd);
	//struct Page *pfp = (struct Page*) va2page(curenv->env_pgdir, p);
	do{
		runs = env->env_runs;
		pfd = pageref(fd);
		pfp = pageref(p);
	}while (runs != env->env_runs);//since read and write function may shift, we need to wait until no shift happens 	
	if (pfd == pfp) {
		return 1;
	}
	return 0;
...
piperead(struct Fd *fd, void *vbuf, u_int n, u_int offset)
{
	...
	return i;
/*	if (_pipeisclosed(fd,p)) {
		return i;//terminate normally
	} else {
		user_panic("pipe still opened after reading");
		return -E_INVAL;
	}*/
...
/*	if (_pipeisclosed(fd,p)) {
	...
	user_panic("pipe still opened after writing");*/	
...
{
	//should unmap fd before pipe to avoid pageref(pipe) = pageref(p[0])
