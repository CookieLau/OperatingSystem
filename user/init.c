...
umain(int argc, char **argv)
{
	...
	if (r != 0) {
		...
		writef("the first fd_id is %c, fd_offset is %d, file_name is %s, file_type is %d, file_dir %s\n",fd->fd_dev_id, fd->fd_offset,filefd->f_file.f_name,filefd->f_file.f_type, filefd->f_file.f_dir->f_name);
		fd = num2fd(1);
		filefd = (struct Filefd*)fd;
		writef("the first fd_id is %c, fd_offset is %d, file_name is %s, file_type is %d, file_dir %s\n",fd->fd_dev_id, fd->fd_offset,filefd->f_file.f_name,filefd->f_file.f_type, filefd->f_file.f_dir->f_name);	
