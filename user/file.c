...
open(const char *path, int mode)
{
	...
	if (debug)
		writef("here open path:%s, alloc a fd\n",path);
	if ((r = fd_alloc(&fd)) < 0) {
		writef("error in user/file.c/open fd_alloc\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
		return r;
	}
	if (debug)
		writef("new alloced fd is %d file is %s\n",fd2num(fd),path);
	// Step 2: Get the file descriptor of the file to open.
	if ((r = fsipc_open(path,mode,fd)) < 0) {
		writef("error in user/file.c/open fsipc_open\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
		return r;
	}
	if (debug && strcmp(path,"outputfile") == 0 && mode == 0)
		user_panic("got u~~\n");
	if (debug)
		writef("here open the path:^%s$ with mode:%d and fd:%d\n",path,mode,num2fd(fd));
	...
	for (i=0;i<size;i+=BY2BLK) {
		/*if ((r = syscall_mem_alloc(0, va+i, PTE_R|PTE_V)) < 0) {
			writef("error in user/file.c/open syscall mem alloc\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
			return r;
		}*/
		if ((r = fsipc_map(fileid, i, va+i)) < 0) {
			writef("error in user/file.c/open fsipc map\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
			return r;
		}
	}
...
read_map(int fdnum, u_int offset, void **blk)
{
	...
	if ((r = fd_lookup(fdnum, &fd)) < 0) {
		writef("cannot find any fd with fdnum:%d\n",fdnum);
		return r;
	}

	if (fd->fd_dev_id != devfile.dev_id) {
		writef("fd_dev_id not match with dev_id");
		return -E_INVAL;
	}
	...
	if (offset >= MAXFILESIZE) {
		writef("offset exceeds\n");
		return -E_NO_DISK;
	}

	if (!((* vpd)[PDX(va)]&PTE_V) || !((* vpt)[VPN(va)]&PTE_V)) {
		writef("page table not mapped\n");
		return -E_NO_DISK;
	}
