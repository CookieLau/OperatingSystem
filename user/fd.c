...
read(int fdnum, void *buf, u_int n)
{
	...
	if ((r = fd_lookup(fdnum,&fd))< 0
		|| (r = dev_lookup(fd->fd_dev_id, &dev)) < 0) {
			writef("error in user/fd.c read step 1\n");
			return r;
		}
...
/*	if (r > 0) {
		fd->fd_offset -= r;
		*((char *)buf + r) = '\0';//since n is byte --> 8 bits
	}*/
	if (r >= 0) {
		fd->fd_offset += r;
		*((char *)buf+r) = '\0';
...
write(int fdnum, const void *buf, u_int n)
{
	...
	struct Fd *fd;
	if (debug) 
		writef("enter write function and fdnum is %d\n",fdnum);
	if ((r = fd_lookup(fdnum, &fd)) < 0
		||  (r = dev_lookup(fd->fd_dev_id, &dev)) < 0) {
		if (debug) 
			writef("wrong in write function, cannot find fd:%d or its dev\n",fdnum);
		return r;
	}
	...
	if ((fd->fd_omode & O_ACCMODE) == O_RDONLY) {
		if (debug)
			writef("DEBUG------file:%s in fd.c write function meet RDONLY when write,the fd is %d o_mode is %d\n",filefd->f_file.f_name,fdnum,fd->fd_omode);
	...
	r = (*dev->dev_write)(fd, buf, n, fd->fd_offset);//r returns n (bytes) in normal-behavior, returns 0 in bad-behavior
